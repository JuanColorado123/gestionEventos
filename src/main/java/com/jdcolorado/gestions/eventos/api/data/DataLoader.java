...
import com.jdcolorado.gestions.eventos.api.domain.*;
import com.jdcolorado.gestions.eventos.api.repository.*;
    ...
    public void run(String... args) throws Exception {
        ...
        if (eventRepository.count() == 0) { // Solo cargar eventos si la tabla está vacía
            ...
            for (int i = 1; i <= 60; i++) { // Cambia 60 al número deseado de eventos
                ...
                if (i % 2 == 0) {
                    event.addAttendSpeaker(john); // Usamos el método addSpeaker para manejar la relación
                } else {
                    event.addAttendSpeaker(jane);
                }
                // Si quieres que algunos tengan ambos oradores:
                if (i % 5 == 0) {
                    event.addAttendSpeaker(john);
                    event.addAttendSpeaker(jane);
                }
